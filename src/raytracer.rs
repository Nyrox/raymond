...
impl<'a> Raytracer<'a> {
    fn trace(&self, ray: &Ray, camera_pos: Vector3<F>, bounces: usize) -> (F, Vector3<F>) {
        ...
        for i in 0..N {
            if material_metalness > 0.9 { continue; }

            let sample = self.uniform_sample_hemisphere();
            let sample_world = (local_cartesian_transform * sample).normalize();

            let incoming = self.trace(&Ray { origin: hit.position + sample_world * 0.0001, direction: sample_world }, hit.position, bounces - 1);
        ...
        let reflect = (-view_dir - 2.0 * (-view_dir.dot(hit.normal).max(0.0) * hit.normal)).normalize();
        for i in 0..N {
